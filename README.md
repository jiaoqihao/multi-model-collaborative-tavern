...
系统采用按角色隔离的信息流：每个角色只接收自己的完整角色卡、相关记忆和获准感知的事件；统一上下文组装器按 Token 预算、相关性、重要性、时效和生命周期选择证据。长期状态由事件溯源、剧情线索、场景／章节摘要和带有效区间的角色记忆共同维护，父回合指针与完整快照提供可回退、可重新生成且不会串线的剧情分支。
...
数据库使用 `stories`（用户、配置、根快照、当前快照、版本号）、`turns`（父回合、正文、角色快照、分发记录）、`profiles`（用户、模型配置、加密密钥）、`presets`（提示词预设）、`character_library`（可复用角色模板），以及追加式 `story_events`、`story_summary_records`、`story_thread_records` 审计表。故事内角色仍在 JSON 快照中按 ID 独立保存；导入模板时创建新的故事角色 ID。
...
完整快照和父回合指针支持分支恢复。信息检索使用关键词（包含中文二元词片段）、标签、重要性、时效和生命周期，并在 Token 预算内组装；尚未使用向量数据库。每 8 回合或转场生成场景摘要，每 4 个场景摘要汇总为章节摘要，均保留来源回合。用户可以编辑角色状态，编辑只影响当前进度之后的新回合。
...
- 每个故事最多保存 150 个回合版本，适合验证产品流程；已有结构化剧情线索和分层摘要，但尚未实现历史归档或向量检索。原始经历列表仍保留在回合快照中。
