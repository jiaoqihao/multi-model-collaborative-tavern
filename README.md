...
数据库使用 `stories`（用户、配置、根快照、当前快照、版本号）、`turns`（父回合、正文、角色快照、分发记录）、`profiles`（用户、模型配置、加密密钥）、`vector_connections`（按用户保存加密的 Qdrant 连接）、`presets`（提示词预设）、`character_library`（可复用角色模板）、`memory_vectors`（按模型版本隔离的角色记忆向量及 Qdrant 回退缓存），以及追加式 `story_events`、`story_summary_records`、`story_thread_records` 审计表。故事内角色仍在 JSON 快照中按 ID 独立保存；导入模板时创建新的故事角色 ID。
...
在“模型与连接”先保存一个支持 embeddings 的 OpenAI、Gemini 或 OpenAI 兼容接口；聊天模型 ID 与向量模型 ID 可以不同。在“故事与我的角色”选择“关键词 + 向量混合检索”、该连接及准确的向量模型 ID，再测试接口并保存故事。默认存储仍是内置 D1；若要使用专用向量数据库，先自行创建 Qdrant Cloud 实例，在“模型与连接”保存其 HTTPS 地址和 API 密钥，再在故事设置中把“向量存储”切换到 Qdrant 并选择连接。可按需点击“补建一批向量索引”；后续真实生成也会为被唤醒角色逐批补建索引。嵌入调用可能计费。DeepSeek 的聊天接口不能直接用作向量模型，若使用 DeepSeek 负责叙事，仍需另配支持 embeddings 的接口。Qdrant 连接密钥仅在服务端加密保存，不放在故事快照或工作台响应中。

索引只保存记忆 ID、内容指纹和向量，不另存角色卡原文。查询必须同时匹配用户、故事、角色、模型配置，并且候选记忆必须存在于当前剧情分支的快照与祖先路径中。Qdrant 查询还会用当前分支允许的点 ID 过滤，返回后再核验元数据与内容指纹；远端集合被清空后可逐批补建。记忆变更时指纹不一致会重新嵌入；其他分支的向量不会进入本轮排序。索引覆盖低于 75% 时继续使用关键词结果；Qdrant 不可用时退回 D1 混合检索，D1 或 embeddings 接口不可用时退回关键词检索，正文生成不会因此中断。剧情版本中可查看该角色本轮是否实际使用混合检索及索引覆盖数。

内置 D1 在单个角色记忆集合上做精确余弦排序；可选 Qdrant 通过 HTTPS REST 提供专用向量索引，同时保留 D1 缓存用于回退。没有 Qdrant 实例时无需配置，原有故事保持 D1 模式。
