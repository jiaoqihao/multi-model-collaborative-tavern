...
export type PresetSelection = {director:string; actor:string; narrator:string};
...
export type StoryState = { title: string; world: string; opening: string; player: string; playerPersona: string; playerState: {location:string;clothing:string;condition:string;inventory:string}; style: string; presetSelection?:PresetSelection; characters: Character[] };
...
export type Turn = { id: string; parentId: string | null; input: string; mode: Mode; narrative: string; createdAt: string; demo: boolean; snapshot: StoryState; trace: { selected: string[]; events: string[]; deliveries: { characterId: string; visible: string }[]; presets?:{stage:string;name:string;revision:number;warnings:string[];characters:number}[] } };
