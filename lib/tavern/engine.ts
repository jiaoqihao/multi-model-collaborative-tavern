import { z } from "zod";
import type { Character, GenerationStage, Mode, StageTrace, StoryState, Turn } from "./types";
...
async function jsonCall<T>(call:CallModel,id:string,system:string,input:unknown,schema:z.ZodType<T>,context:PresetContext|undefined,stage:string,diagnostics?:StageTrace[],modelLabel=id):Promise<T>{
 ...
 for(let attempt=0;attempt<2;attempt++){
  ...
  try { const value=schema.parse(parseModelObject(text));diagnostics?.push({stage,modelId:id,modelLabel,attempts:attempt+1,durationMs:Date.now()-started,cached:false,inputCharacters,estimatedInputTokens:Math.ceil(inputCharacters/2.4),outputCharacters:text.length});return value; }catch(error) {
...
export function retrieveMemories(c:Character,visible:string){
 const text=visible+" "+c.state.location+" "+c.state.goal; const segments=text.match(/[\p{L}\p{N}]{2,}/gu)||[]; const words=[...new Set(segments.flatMap(w=>/[\u4e00-\u9fff]/.test(w)?Array.from({length:Math.max(0,w.length-1)},(_,i)=>w.slice(i,i+2)):[w.toLowerCase()]))].slice(0,180);
 const queryTags=new Set([c.name,...segments.filter(w=>w.length<=40)]);
 return c.memories.filter(m=>(m.status||"active")==="active").map((m,i)=>({m,score:m.importance*3+(i+1)/Math.max(1,c.memories.length)*2+words.filter(w=>m.content.toLowerCase().includes(w)).length*4+(m.tags||[]).filter(t=>queryTags.has(t)).length*5+(m.kind==="observation"?1:0)})).sort((a,b)=>b.score-a.score).slice(0,16).map(x=>x.m);
}
...
function appendMemory(c:Character,memory:Character["memories"][number]){const duplicate=[...c.memories].reverse().find(m=>(m.status||"active")==="active"&&m.kind===memory.kind&&m.content.trim()===memory.content.trim());if(duplicate){duplicate.status="superseded";memory.supersedes=[duplicate.id]}c.memories.push(memory)}
...
export async function runTurn(args:{story:StoryState;history:Turn[];input:string;mode:Mode;directorId:string;turnId:string;call:CallModel;regenerate?:boolean;progress?:(text:string)=>void;cache?:StageCache;describeModel?:(id:string)=>string}){
 ...
 const plan=await stageCall("director",call,directorId,
  ...
  {world:story.world,opening:story.opening,player:story.player,playerPersona:story.playerPersona,playerState:story.playerState,cast:publicCast,characterCards,recentEvents:history.slice(-8).map(t=>t.trace.events),input,mode},planSchema,directorContext,"导演组织场景",stageArgs);
 ...
 const settlementId=stageModel("settlement");const settled=await stageCall("settlement",call,settlementId,
  ...
  {world:story.world,player:story.player,playerState:story.playerState,cast:publicCast,characterCards,input,mode,deliveries:plan.deliveries,responses:responses.map(({characterId,speech,actionIntent})=>({characterId,speech,actionIntent}))},resultSchema,buildPresetContext("settlement",story,history,input,undefined,trigger),"协调事件与状态",stageArgs);
 ...
 const narratorId=stageModel("narrator");const output=await stageCall("narrator",call,narratorId,
 ...
 for(const c of snapshot.characters){
  ...
  for(const {event,eventIndex} of events)appendMemory(c,{id:crypto.randomUUID(),turnId,content:event.description,kind:"observation",importance:3,source:{type:"event",eventIndex},tags:memoryTags(event.description,story),status:"active"});
  if(response?.thought)appendMemory(c,{id:crypto.randomUUID(),turnId,content:response.thought,kind:"belief",importance:2,source:{type:"inference"},tags:memoryTags(response.thought,story),status:"active"});
 }
 ...
 await Promise.all(affected.map(async c=>{
  ...
  const schema=z.union([cardMemoryDeltaSchema,cardMemoryUpdateSchema]).superRefine((update,ctx)=>{try{"summary" in update?writeCardMemory(c.card!,update,c.state,turnId,"model"):writeCardMemoryDelta(c.card!,update,c.state,turnId,"model")}catch(error){ctx.addIssue({code:"custom",message:(error as Error).message})}});
 ...
 return {snapshot,narrative:output.narrative,trace:{selected:plan.deliveries.map(d=>d.characterId),events:settled.events.map(e=>e.description),deliveries:plan.deliveries.map(({characterId,visible})=>({characterId,visible})),stages:diagnostics}};
}
