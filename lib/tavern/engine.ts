...
import { economyMemoryUpdate } from "./economy-memory";
...
const characterRef=(ids:Set<string>)=>ids.size?z.enum([...ids] as [string,...string[]]):z.never();
const planSchema=(limit:number,ids:Set<string>)=>z.object({deliveries:z.array(z.object({characterId:characterRef(ids),visible:line,direction:line})).max(limit)}).superRefine((plan,ctx)=>{
 if(new Set(plan.deliveries.map(item=>item.characterId)).size!==plan.deliveries.length)ctx.addIssue({code:"custom",path:["deliveries"],message:"同一角色只能分配一次"});
});
const actorSchema=z.object({speech:line,actionIntent:line,emotion:line,thought:line,goal:line,relationship:line});
const resultSchema=(ids:Set<string>)=>z.object({events:z.array(z.object({description:line,visibleTo:z.array(characterRef(ids)).max(6),visibleToPlayer:z.boolean()})).min(1).max(12),changes:z.array(z.object({characterId:characterRef(ids),location:line.optional(),clothing:line.optional(),condition:line.optional()})).max(6),playerChanges:z.object({location:line.optional(),clothing:line.optional(),condition:line.optional(),inventory:line.optional()}).optional(),threads:z.array(z.object({action:z.enum(["open","resolve"]),description:z.string().min(1).max(1000),characterIds:z.array(characterRef(ids)).max(6),sourceEventIndex:z.number().int().nonnegative().optional()}).strict()).max(8).default([])}).superRefine((result,ctx)=>{
 if(new Set(result.changes.map(item=>item.characterId)).size!==result.changes.length)ctx.addIssue({code:"custom",path:["changes"],message:"同一角色的状态变化只能出现一次"});
 result.threads.forEach((thread,index)=>{if(thread.sourceEventIndex!==undefined&&thread.sourceEventIndex>=result.events.length)ctx.addIssue({code:"custom",path:["threads",index,"sourceEventIndex"],message:"必须引用本轮 events 中存在的事件索引"});});
});
...
function mergeStoryThreads(story:StoryState,updates:z.infer<ReturnType<typeof resultSchema>>["threads"],turnId:string){
...
export async function runTurn(args:{story:StoryState;history:Turn[];input:string;mode:Mode;directorId:string;turnId:string;call:CallModel;memorySearch?:(character:Character,query:string,budgetTokens:number)=>Promise<{items:Character["memories"];usedVector:boolean;indexed:number;total:number}>;regenerate?:boolean;progress?:(text:string)=>void;cache?:StageCache;describeModel?:(id:string)=>string}){
 ...
 const plan=await stageCall("director",call,directorId,
  '你负责信息分发。选择本轮需要回应的 0–4 个角色。characterCards 是按角色隔离的卡片与累积记忆，不是所有人的共同知识；不得把某人的私密记忆直接分发给其他角色。根据距离、在场情况和交谈音量，为每人生成各自实际感知的 visible。低声、密信、私聊只对接收人披露内容；其他人至多看到外部动作。不复制完整用户输入给无关角色。director 输入是幕后创作要求，只在 direction 下发该角色必要的表演要求，不作为听见的台词。roleplay 输入中未明说的想法不能成为事实。不得凭空添加玩家行动。visible 只转述本轮已提供的感知，不得提前编写角色接下来要说的话或要做的动作。roleplay 模式的 direction 留空，不得把提问猜测为试探、敌意或预设拒绝回答；角色反应由角色阶段自主决定。没有获得新信息且无需回应的角色不要唤醒。输出 {"deliveries":[{"characterId":"id","visible":"本角色看见听见的内容","direction":"必要的幕后表演要求，没有则为空"}]}。',
  {world:story.world,opening:story.opening,player:story.player,playerPersona:story.playerPersona,playerState:story.playerState,cast:publicCast,characterCards,contextHistory:directorContext.history.filter(message=>message.role==="assistant"),sceneSummaries:story.context?.sceneSummaries.slice(-3)||[],openThreads:story.context?.threads.filter(thread=>thread.status==="open").slice(-12)||[],input,mode,collaborationMode,maxRespondingCharacters:deliveryLimit},planSchema(deliveryLimit,ids),directorContext,"导演组织场景",stageArgs);
 ...
 // Roleplay authorizes perception routing, not unsolicited acting instructions.
 // Apply also to cached plans; only explicit director mode may guide performance.
 if(mode==="roleplay")for(const delivery of plan.deliveries)delivery.direction="";
 ...
 const settlementId=stageModel("settlement");const settlementContext=buildPresetContext("settlement",story,history,input,undefined,trigger);let settled=await stageCall("settlement",call,settlementId,
  ...
  {world:story.world,player:story.player,playerState:story.playerState,cast:publicCast,characterContext:characterCards,contextHistory:settlementContext.history.filter(message=>message.role==="assistant"),input,mode,deliveries:plan.deliveries,responses:responses.map(({characterId,speech,actionIntent})=>({characterId,speech,actionIntent})),openThreads:story.context?.threads.filter(thread=>thread.status==="open")||[]},resultSchema(ids),settlementContext,"协调事件与状态",stageArgs);
 ...
 if(collaborationMode==="precise"&&complexityScore>=3){args.progress?.("正在复核冲突与状态一致性…");settled=await stageCall("review",call,settlementId,
  '你是事件一致性复核器。核对 candidate 是否满足输入、角色行动、已知状态、信息可见范围和玩家自主性。deliveries 是各角色收到的信息，responses 是实际回应和动作意图；它们不是已成立的客观结果。以原始 input 核对玩家行为，不接受导演或候选事件增写的玩家行动。修正互斥结果、物品归属冲突、越权知情或追溯改写；没有问题则原样返回。只返回与事件裁决相同的结构。threads 是增量指令，只使用 action（open 或 resolve）、description、characterIds 及可选 sourceEventIndex，不能照抄 openThreads 中的 id、status 等持久化字段。',
  {world:story.world,player:story.player,playerState:story.playerState,cast:publicCast,input,mode,deliveries:plan.deliveries,responses:responses.map(({characterId,speech,actionIntent})=>({characterId,speech,actionIntent})),candidate:settled,openThreads:story.context?.threads.filter(thread=>thread.status==="open")||[]},resultSchema(ids),settlementContext,"一致性复核",stageArgs);for(const event of settled.events)ensureIds(event.visibleTo,ids);ensureIds(settled.changes.map(change=>change.characterId),ids);for(const thread of settled.threads||[]){ensureIds(thread.characterIds,ids);if(thread.sourceEventIndex!==undefined&&thread.sourceEventIndex>=settled.events.length)throw new AppError("剧情线索引用了不存在的事件，本轮未保存。",502);}if(new Set(settled.changes.map(change=>change.characterId)).size!==settled.changes.length)throw new AppError("状态变更重复，本轮未保存。",502);}
 ...
 const affected=snapshot.characters.filter(c=>plan.deliveries.some(d=>d.characterId===c.id)||settled.events.some(e=>e.visibleTo.includes(c.id))||settled.changes.some(change=>change.characterId===c.id)||threadChanges.some(thread=>thread.characterIds.includes(c.id)));
 if(affected.length)args.progress?.(collaborationMode==="economy"?"正在整理本轮记忆…":"主 AI 正在更新角色卡记忆…");
 if(collaborationMode==="economy")for(const c of affected){const visibleEvents=settled.events.filter(event=>event.visibleTo.includes(c.id)).map(event=>event.description);c.card=writeCardMemory(c.card!,economyMemoryUpdate(readCardMemory(c.card!),visibleEvents,threadChanges,c.id),c.state,turnId,"model");}
