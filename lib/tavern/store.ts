...
export async function vectorConnectionRows(user:string){return (await db().prepare("SELECT id,name,base_url,encrypted_key,revision FROM vector_connections WHERE owner=? ORDER BY rowid DESC").bind(user).all<{id:string;name:string;base_url:string;encrypted_key:string;revision:number}>()).results;}
...
export async function getWorkspace(user:string,id?:string):Promise<Workspace>{
 ...
 return {id:row.id,stories:listing,revision:row.revision,story:storySchema.parse(JSON.parse(row.data)),headId:row.head_id,turns:(await allTurns(row.id)).map(t=>({...t,snapshot:storySchema.parse(t.snapshot)})),profiles,vectorConnections:(await vectorConnectionRows(user)).map(row=>({id:row.id,name:row.name,baseUrl:row.base_url,revision:row.revision,hasKey:!!row.encrypted_key})),presets:(await presetRows(user)).map(summarizePreset),characterLibrary:await characterLibraryRows(user),directorId:row.director_id,demo:!!row.demo};
}
