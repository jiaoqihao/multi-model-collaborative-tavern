...
const cardSchema=z.object({format:z.enum(["yaml","json"]),filename:z.string().max(180),source:z.string().max(60000)}).strict().superRefine((card,ctx)=>{try{readCardMemory(card)}catch(error){ctx.addIssue({code:"custom",message:(error as Error).message})}});
...
export const characterSchema=z.object({id:z.string().min(1).max(80),name:z.string().trim().min(1).max(40),role:short,persona:z.string().max(6000),secret:z.string().max(4000),color:z.enum(["amber","blue","gray"]),modelId:z.string().max(100),presetId:z.string().max(100).default("inherit"),card:cardSchema.optional(),state:stateSchema,memories:z.array(memorySchema).max(2000)}).strict();
export const storySchema=z.object({title:z.string().trim().min(1).max(80),world:z.string().max(12000),opening:z.string().max(12000),player:z.string().trim().min(1).max(40),playerPersona:z.string().max(6000),playerState:playerStateSchema.default({location:"当前场景",clothing:"日常服饰",condition:"健康",inventory:""}),style:z.string().max(4000),presetSelection:z.object({director:z.string().max(100),actor:z.string().max(100),narrator:z.string().max(100)}).strict().default({director:"",actor:"",narrator:""}),characters:z.array(characterSchema).max(6)}).strict().refine(s=>new Set(s.characters.map(c=>c.id)).size===s.characters.length,"角色 ID 不可重复");
