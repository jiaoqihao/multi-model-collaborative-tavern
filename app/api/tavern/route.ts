import { z } from "zod";
import { AppError, ancestry, profileSchema, storySchema, turnSchema } from "@/lib/tavern/validation";
import { allTurns, commitTurn, createStory, db, encryptionSecret, getStory, getWorkspace, owner, profileRows, requireOrigin } from "@/lib/tavern/store";
...
export async function POST(request:Request){try{
 requireOrigin(request);const user=owner(request);const raw=await request.text();if(raw.length>300000)throw new AppError("请求内容过长。",413);let body;try{body=JSON.parse(raw)}catch{throw new AppError("请求不是有效的 JSON。");}
 ...
 if(action==="deleteProfile"){
  ...
  if(using)throw new AppError("这个模型仍被故事或角色使用，请先更换分配。");
 ...
 const call=async(id:string,system:string,prompt:string)=>{const row=profiles.find(p=>p.id===id);if(!row)throw new AppError("角色或主 AI 的模型配置不存在，请检查模型分配。");if(!keys.has(id))keys.set(id,row.encrypted_key?await cryptKey(row.encrypted_key,encryptionSecret(),true):"");return complete(JSON.parse(row.data) as Profile,keys.get(id)!,system,prompt);};
 const stream=new ReadableStream({async start(controller){
  ...
  try{
   ...
   const result=row.demo?demoTurn(story,args.input,args.mode,args.requestId):await runTurn({story,history:ancestry(turns,parentId),input:args.input,mode:args.mode,directorId:row.director_id,turnId:args.requestId,call,progress:message=>send("progress",{message})});
   const turn:Turn={id:args.requestId,parentId,input:args.input,mode:args.mode,createdAt:new Date().toISOString(),demo:!!row.demo,...result,snapshot:storySchema.parse(result.snapshot)};
